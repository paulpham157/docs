...
Service accounts are not supported through the Claude web connector. Use [Claude Code](#claude-code-1) or [Cursor](#cursor-1) instead.
...
Service accounts are not supported through the ChatGPT connector. Use [Cursor](#cursor-1) or another client that supports custom headers instead.
