...
<Callout type="info">
  To invite and manage users in your organization and project, you will need an
  admin or owner organization role.
</Callout>
...
1. Under "Organization Settings", click **Users & Teams**, and then the **Invited Users** tab.
2. After locating the user's email address, click the 'Resend Invite' button located on the right side of the screen.
3. A pop-up window appears, giving you the option to either automatically resend the invite email by clicking the "Resend" button or copy a link to manually send it to the user, in case they have problems receiving our emails.
...
<Callout type="info">
  Deleting a user from an organization will remove them from all projects and
  teams within the organization and is non-reversible.
</Callout>
...
<Callout type="info">
  Only users with Organization Owner or Organization Admin roles can remove
  users who have access to a project granted through a
  [Team](/docs/orgs-and-projects/roles-and-permissions#teams).
</Callout>
...
If all existing owners are no longer working with the company and you need to assign a new owner, please [reach out to the support team](https://mixpanel.com/get-support).
...
<Callout type="info">
  Custom Roles is available to organizations on **Enterprise** plans. See our
...
1. Under "Project Settings", click **Project Users**.
2. Select the **Project Roles** tab to see a list of current roles in the project.
3. Click **Create Custom Role**.
4. Enter a **Name** and optional **Description** for the role. Names must be unique within the project.
5. Optionally, select an existing role as a template to start with its permissions pre-selected.
6. Configure permissions by toggling individual permissions on or off across the available categories (see [Permission Categories](#permission-categories) below).
7. Click **Save** to create the role.

![create_custom_role](/create_custom_role.png)
...
| **Data Management**                   | Edit Metadata, Set Event and Property Visibility, Data Verification, Manage Schemas, Manage Lookup Tables, Mark Data Classified, Edit Profiles, Data Deletion Dropping and Merging, Data Settings Access |
| **Data Access, Exports, and Alerts**  | Public Boards, Classified Data Access, Export Lexicon, Export Reports as CSV, Export Users as CSV, Export Events as CSV, Create Alerts, Create Board Subscriptions, Manage All Alerts                    |
...
<Callout type="info">
  All custom roles automatically include base permissions: viewing reports,
  editing visibility of entities you created, and sharing entities you created.
  These permissions cannot be removed.
</Callout>
...
<Callout type="info">
  Cloning a role to another project requires the **manage custom roles**
  permission on the target project. Any permissions gated by features that are
  not enabled on the target project will be automatically excluded from the
  cloned role.
</Callout>
...
<Callout type="info">
  When a custom role is deleted, all users currently assigned to that role will
  be automatically reassigned to the **Consumer** role. Review the list of
  affected users before confirming deletion.
</Callout>
...
1. Navigate to **Organization Settings** → **Users & Teams** → **Teams**.
2. Select the team you want to configure.
3. Under **Projects**, add the project or find an existing project assignment.
4. Click the role dropdown for that project and select the custom role.

All members of the team inherit the assigned role for that project.

### Changes to Classified Data Access (April 2026)

Previously, classified data access was managed through a separate per-user toggle on the Project Users page. Admins could individually grant or revoke a user's ability to view sensitive properties, independent of their project role.

With Custom Roles, classified data access is now a permission called **Classified Data Access** under the **Data Access, Exports, and Alerts** category. This gives you a single place to define what a role can and cannot do, rather than managing access through a separate toggle.

No default role — including Owner and Admin — has the ability to view classified data by default. This is intentional: access to classified data should always be an explicit decision, not an inherited side effect of a role. Owners and Admins retain the ability to control who gets classified data access, but must be explicitly granted viewing access themselves through a custom role.

Note the distinction between two related permissions:

- **Mark Classified Data** (under Data Management) — Controls which properties are classified as sensitive. Does not grant the ability to view that data.
- **Classified Data Access** (under Data Access, Exports, and Alerts) — Grants the ability to view properties that have been marked sensitive.
...
#### Why can't Owners or Admins see classified data by default?

By design, no role can view classified data by default — including Owner and Admin. Owners and Admins can control who gets access, but cannot view classified data themselves unless explicitly granted via a custom role. This ensures classified data access is always intentional.
