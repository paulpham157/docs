...
- Group cohorts are created in the same [cohort builder](/docs/users/cohorts), scoped to the relevant group key.
...
<Callout type="info">
    For detailed configuration options and examples, see our [Runtime Events guide](/docs/featureflags/runtime-events).
</Callout>
...
> #### 📝 Note: Selecting QA testers on Original ID Merge
>
> This behavior applies only to projects on **Original ID Merge (V2)**. Projects on **Simplified ID Merge (V3)** use a deterministic canonical user ID and do not require manual QA tester maintenance.
>
> When you add a user as a QA tester (by email or distinct_id), Mixpanel saves a snapshot of that user's **most recent `$identified_id`** at the moment of addition, pulled from their `$identify` event history. At flag evaluation time, the `/flags/` endpoint performs an exact string match between the distinct_id your SDK sends and this saved value — there is no cluster-aware resolution at runtime.
>
> **What this means:** if the user later identifies with a different ID than what was saved (e.g., your app's identification logic changes, the user transitions between anonymous and identified states, or your hashing scheme is updated), the saved QA tester record becomes stale, and the flag will no longer return for that user.
>
> **To recover:** remove the QA tester from the flag's configuration and re-add them. The latest `$identified_id` will be snapshotted again.
>
> **Best practice:**
> - Call `identify()` with a stable, consistent value for each logged-in user across sessions.
> - Add QA testers only after the target user has identified at least once with that stable value.
>
> **Debugging tip:** to verify which ID was saved for a QA tester, inspect the `/flags/` network request in your browser's developer tools. The response payload includes a `test_users` field listing the saved IDs. Compare these against the `distinct_id` your SDK is sending — if they don't match, that's the source of the issue. You can also see this in the dropdown when searching for users.
...
<Callout type="info">
Flag persistence is a client SDK feature. It is officially supported on the Android, Swift, Flutter, and JavaScript (Web) client SDKs. Support for React Native is coming soon.
</Callout>
...
<Cards>
  ...
  <Cards.Card icon title="Flutter" href="/docs/tracking-methods/sdks/flutter/flutter-flags" />
  <Cards.Card icon title="React Native (Beta)" href="/docs/tracking-methods/sdks/react-native/react-native-flags" />
</Cards>
...
**Mixpanel Feature Flags with a Customer Data Platform (CDP)**
...
**Mixpanel Feature Flags with Google Tag Manager (GTM)**
You can implement Mixpanel Feature Flags through Google Tag Manager. See the [Google Tag Manager integration guide](/docs/tracking-methods/integrations/google-tag-manager) for step-by-step instructions on initializing flags, evaluating them, and tracking exposure events via GTM.
...
To duplicate a feature flag to another project, open the Other Environments menu and select **Duplicate To Project**. In the dialog, choose the destination project.

This flow is for duplicating across projects only — to duplicate within the same project, use the same **Duplicate** option within flag's overflow (`...`) menu in the top-right of the flag page .
...
To duplicate a flag to another project, you need permission to create and edit feature flags in **both** the source and destination projects. The required permission level in the destination project is the same as what's needed to manually create a new flag there — governed by that project's roles and custom roles.

#### What to expect after duplicating

When a flag is duplicated to another project, time-based settings reset to the day the duplicate was created — the same behavior as duplicating within a project.

If your flag targets a cohort, Mixpanel assumes the event properties referenced by that cohort exist in the destination project. It does not validate property availability at copy time, so verify that your destination project collects the same events and properties before using the flag there.

#### Removing a connection

Removing an inter-project connection from the "Other Environments" menu unlinks the two flags — it does **not** delete the flag in the destination project. The copied flag remains in the destination project and continues to function independently; it simply loses its link to the original.
...
"Assignment Key" of a flag to a data group key, both the source and destination projects _must_ include the referenced data group in order to duplicate or push changes.
